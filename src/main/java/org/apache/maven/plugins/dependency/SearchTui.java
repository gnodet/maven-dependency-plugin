...
class SearchTui {
    ...
    private void renderSearchBar(Frame frame, Rect area) {
        ...
        String title = loading ? " Searching\u2026 " : " Maven Central Search ";
    ...
    private void renderInfoBar(Frame frame, Rect area) {
        ...
        renderArtifactDetails(frame, rows.get(0));
        renderSyntaxHelp(frame, rows.get(1));
    ...
    private void triggerSearch() {
        ...
        CompletableFuture.supplyAsync(() -> {
                    try {
                        return client.query(searchQuery, 20, 0);
                    } catch (Exception e) {
                        return null;
                    }
                })
     ...
     * (start >= rows returns 0 docs), so we re-query with a larger rows value.
     */
    ...
    private void fetchMoreResultsSync() {
        ...
        try {
            JsonObject response = client.query(lastSearchedQuery, currentSize + 20, 0);
    ...
    private void prefetchMoreResults() {
        ...
        CompletableFuture.supplyAsync(() -> {
                    try {
                        return client.query(searchQuery, currentSize + 20, 0);
                    } catch (Exception e) {
                        return null;
                    }
                })
    ...
    static List<String[]> extractArtifacts(JsonObject responseBody) {
